...
```{r}
bikestype <- bike_rides %>% group_by(rideable_type,Weekly = floor_date(Ymd,"week")) %>%
                summarise(
                  Minutes = sum(Minutes),
                  Mean = mean(Minutes),
                  Median = median(Minutes),
...
```{r}
 # bikes_type_wbikestype %>% group_by(Weekly = floor_date(Ymd,"week"),rideable_type) %>%     summarise(Total = sum(Count)) 
```
